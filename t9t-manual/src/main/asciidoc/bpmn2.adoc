...
The implementation to use is defined using an extension `camunda:properties` with the name `WorkflowType` attached to the triggered start event in process definition.
...
In case the `mayRun` or `execute` method of the workflow step throws an `ApplicationException`, the return value is provided as `ERROR`.
Furthermore, a BPM error with the exceptions error code and error message is created, which can be handled using an error boundary event.
...
The return code of the execution is checked and in case of an error code, an BPM error is throws with the return code and return message, which can be handled using
