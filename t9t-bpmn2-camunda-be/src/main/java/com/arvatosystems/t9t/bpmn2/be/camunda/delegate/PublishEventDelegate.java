...
public class PublishEventDelegate implements JavaDelegate {
    ...
    private Expression eventJson;

    @Override
    public void execute(DelegateExecution execution) throws Exception {
        final EventParameters eventParameters = requireNonNull(getValueAsBonaPortable(eventJson, execution, null), "Parameter 'event' must not be empty");

        executor.publishEvent(eventParameters);
    }

    public Expression getEventJson() {
        return eventJson;
    }

    public void setEventJson(Expression eventJson) {
