...
public class JobExecutionRequestWrapper {
     ...
     */
    protected boolean executeJobs(List<String> jobIds, Long tenantRef, UUID apiKey) {
        LOGGER.debug("Prepare job execution request for jobs of tenant ref {} ({} jobs}", tenantRef, jobIds.size());

        Authentication authentication = getAuthenticationForTenantRef(tenantRef, apiKey);
        ServiceResponse response = performJobExecutorRequest(jobIds, authentication);
        ...
        if (response.getReturnCode() == T9tException.JWT_EXPIRED) {
            ...
            response = performJobExecutorRequest(jobIds, authentication);
        }
